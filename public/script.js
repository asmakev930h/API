...
function render(reset = true) {
  ...
  slice.forEach((e) => {
    ...
    const delay = (filtered.indexOf(e) % perPage) * 0.05;
    ...
    card.innerHTML = `
        ...
        <div class="card-footer">
            <div class="status-dot loading" title="Checking status..."></div>
    ...
    checkEndpointStatus(e.path, dot);
  });
...
/* ======  STATUS CHECK  ====== */
async function checkEndpointStatus(path, dotElement) {
    ...
    try {
        ...
        const response = await fetch(url, {
            ...
            // Use 'no-cors' if necessary, but here it's same-origin so valid CORS is expected/handled
        });
        ...
        if (response.status === 200) {
            ...
            dotElement.title = `Offline (${response.status})`;
        }
    } catch (error) {
        dotElement.classList.remove("loading");
        dotElement.classList.add("offline");
        dotElement.title = "Offline (Error/Timeout)";
    }
}
